...
- `D3D11`: Windows 路径。应用创建一个带 `D3D11_CREATE_DEVICE_VIDEO_SUPPORT` 的共享 device，FFmpeg 的 D3D11VA 解码器与 `D3D11Renderer` 共用它。解码帧保留在 GPU，renderer 从解码纹理数组的对应 slice 用 `CopySubresourceRegion` 拷到同格式的可采样 NV12 或 P010 纹理，经 HLSL 按 range 与 BT.601 / BT.709 / BT.2020 NCL 矩阵做 YUV-to-RGB 转换后交给 DXGI swap chain 呈现。未知的 D3D11 surface 格式会安全回退到 Software profile。
...
- `d3d11renderer.h/.cpp`: native Qt child window, DXGI swap chain, GPU NV12/P010 copy and range-aware HLSL rendering.
...
- In D3D11 profile, play H.264/H.265 at 1080p and 4K; verify aspect ratio, full/limited range and BT.601/BT.709/BT.2020 NCL color, resize and seek behavior.
